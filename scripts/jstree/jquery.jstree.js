...
/* 
 * City of Seattle ui plugin 1.0
 * Adds custom ui hover/select behavior.
 */
(function ($) {
	$.jstree.plugin("seaui", {
		__init : function () {
         this.get_container()
            .delegate("a", "click.jstree", $.proxy(function (event) {
						this.click_node(event.currentTarget, event);
					}, this));
		},
		_fn : { 
			click_node : function (obj, event) {
	         if (!this.is_leaf(obj)) {  // is the node clicked a leaf?
	...
	$.jstree.defaults.plugins.push("seaui");
})(jQuery);
//*/
